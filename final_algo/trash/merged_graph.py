...
def print_stats(df_ref, df_ans, G, title=None):
    ...
    for cur_s in df_ans.columns:
        ...
        TP = len(df_ref[real_true & predicted_true])
    ...
    for cur_s in df_ans.columns:
        ...
        for node in selected_nodes:
            ...
            for graph, rev_flag in zip([G, G_rev], [False, True]):
                ...
                s = node
                has_bubble, t = find_superbubble(graph, rev_graph, node)
                while has_bubble:
                    bubbles_s_t.append((s, t, rev_flag))
                    if visited.get(t, False):
                        break
                    visited[t] = True
                    if df_ans.loc[to_my_int(t), cur_s] == 0:
                        ans.append(t)
                    s = t
                    has_bubble, t = find_superbubble(graph, rev_graph, s)
...
def correct_continues(G, df_ans):
    df_corrected_ans = df_ans.copy()
    for cur_s in df_ans.columns:
        selected_nodes = to_double_format(df_ans[df_ans[cur_s] == 1].index)
        for node in selected_nodes:
            if G.out_degree(node) == 1:
                new_ans = to_single_format(list(G.neighbors(node)))
                df_corrected_ans.loc[new_ans, cur_s] = 1
            if G.in_degree(node) == 1:
                new_ans = to_single_format(list(G.predecessors(node)))
                df_corrected_ans.loc[new_ans, cur_s] = 1
...
def main():
    ...
    print_stats(df_ref, df_corrected_cutpoints, G, "CUTPOINTS")

    df_corrected_continues = correct_continues(G, df_corrected_cutpoints)
    print_stats(df_ref, df_corrected_continues, G, "CONTINUES")

    df_corrected_bubbles = correct_bubbles(G, df_corrected_continues, bubbles_s_t, desman_profile)
